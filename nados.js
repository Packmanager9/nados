...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        function continued_stimuli(e) {
            ...
            TIP_engine.y = YS_engine
            TIP_engine.body = TIP_engine
            center.x = TIP_engine.x
            center.y = TIP_engine.y
        }
    }
    ...
    let center = new Circle(350,350, 2, "red")

    class Debris {
        constructor(x,y, color){
            this.body = new Circle(x,y, 1, color)
            this.link = new LineOP(this.body, center)
            this.angle = this.link.angle()

        }
        draw(){
            this.angle = this.link.angle()
            if(this.link.hypotenuse()<200){
                this.angle+=.01//(200-this.link.hypotenuse())/2000
                this.liner = this.link.hypotenuse()
                this.body.y = (Math.sin(this.angle)*this.liner)+center.y
                this.body.x = (Math.cos(this.angle)*this.liner)+center.x
            }   
    ...
    let debris = []
    for(let t = 0;t<700;t++){
        let debra = new Debris(Math.random()*canvas.width, Math.random()*canvas.height, getRandomColor())
